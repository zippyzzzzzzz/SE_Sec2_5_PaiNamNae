...
function drawRouteOnMap(data) {
    ...
    if (data.start?.lat && data.start?.lng) {
        startMarker = new google.maps.Marker({ position: { lat: data.start.lat, lng: data.start.lng }, map: gmap, label: 'A' })
    }
    if (data.end?.lat && data.end?.lng) {
        endMarker = new google.maps.Marker({ position: { lat: data.end.lat, lng: data.end.lng }, map: gmap, label: 'B' })
    }
    // หมุดจุดแวะ (เขียว)
    ...
    if (data.polyline && google.maps.geometry?.encoding) {
        ...
        activePolyline = new google.maps.Polyline({ path, map: gmap, strokeColor: '#2563eb', strokeOpacity: 0.9, strokeWeight: 5 })
        path.forEach(p => bounds.extend(p))
    }
    // fit จากหมุด A/B และจุดแวะ (ในกรณีไม่มี polyline หรือเสริม)
    ...
    if (data.stopsCoords?.length) data.stopsCoords.forEach(s => bounds.extend(new google.maps.LatLng(s.lat, s.lng)))
...
onMounted(async () => {
    ...
    // โหลด Google Maps ถ้ายังไม่ถูกโหลด (อ้างอิง findTrip)
    if (!window.google?.maps) {
    ...
    await fetchRoute()
    if (routeData.value && window.google?.maps) drawRouteOnMap(routeData.value)
})
