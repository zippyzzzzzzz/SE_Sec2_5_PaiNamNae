...
*** Test Cases ***
TC02: Real-time Near Alert Verification (Full Hybrid Flow)
    ...
    ${start_loc}=     Create Dictionary    name=KKU    lat=${13.7563}    lng=${100.5018}
    ...
    FOR    ${i}    IN RANGE    20
        ...
        IF    ${found} == ${TRUE}
            Fail    Negative Test Fail: Notification found on page.
        END
        
        Sleep    3s
        Reload Page
        Log To Console    Check Round ${i+1}: Notification not found as expected.
    END
